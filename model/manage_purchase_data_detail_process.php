...
try {
    ...
    if ($action === 'ADD') {
        ...
        $stmtRunNo = $conn->prepare("SELECT doc_no FROM ims_purchase WHERE doc_no LIKE ? ORDER BY doc_no DESC LIMIT 1");
        ...
        $stmtDelete = $conn->prepare("DELETE FROM ims_purchase_detail WHERE doc_no = ?");
