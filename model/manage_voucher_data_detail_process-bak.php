...
try {
    ...
    if ($action === 'ADD') {
        ...
        $stmt = $conn->prepare("INSERT INTO ims_payment_voucher 
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if (!$stmt->execute([$doc_no, $doc_date, $requester, $supplier_id, $supplier_name, $purpose, $total_amount, $picture_doc, $payment_method, $create_name, $checker_name, $approve_name, $receipt_name])) {
        ...
        $stmt = $conn->prepare("UPDATE ims_payment_voucher 
            ...
            WHERE doc_no = ?");
        if (!$stmt->execute([$doc_date, $requester, $supplier_id, $supplier_name, $purpose, $total_amount, $picture_doc, $payment_method, $create_name, $checker_name, $approve_name, $receipt_name ,$doc_no])) {
            $errorInfo = $stmt->errorInfo();
