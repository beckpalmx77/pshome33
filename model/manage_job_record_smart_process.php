...
## Total number of records without filtering
    $stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM v_jobrecord WHERE 1=1 ");
    $stmt->execute();
    $records = $stmt->fetch();
    $totalRecords = $records['allcount'];

## Total number of records with filtering
    $stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM v_jobrecord WHERE 1=1 " . $searchQuery);
...
## Fetch records
    ...
    $sql_get_data = "SELECT * FROM v_jobrecord WHERE 1=1 " . $searchQuery . " ORDER BY id DESC  " . " LIMIT :limit,:offset";
