...
// Bind values
    ...
    foreach ($empRecords as $row) {
        if ($_POST['sub_action'] === "GET_MASTER") {
            $data[] = array(
                "id" => $row['id'],
                "requester_id" => $row['emp_id'],
                "requester" => $row['f_name'] . " " . $row['l_name'],
            ...
            $data[] = array(
                ...
                "requester_id" => $row['emp_id'],
                "requester" => $full_name,
                "select" => "<button type='button' name='select' id='" . $row['emp_id'] . "@" . $full_name ."' class='btn btn-outline-success btn-xs select' data-toggle='tooltip' title='select'>select <i class='fa fa-check' aria-hidden='true'></i>
                </button>",
            );
        }
    }
