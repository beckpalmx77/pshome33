...
if (isset($_POST["action"]) && $_POST["action"] === 'GET_DATA') {
    ...
    $stmt = $conn->prepare("SELECT * FROM ims_purchase_a_master WHERE id = ?");
    $stmt->execute([$id]);
...
if (isset($_POST["action"]) && $_POST["action"] === 'GET_PURCHASE') {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM ims_purchase_a_master");
    $stmt->execute();
    $records = $stmt->fetch();
    $totalRecords = $records['allcount'];

    // Total records (with filter)
    $stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM ims_purchase_a_master WHERE 1 " . $searchQuery);
    $stmt->execute($searchArray);
    $records = $stmt->fetch();
    $totalRecordwithFilter = $records['allcount'];

    // Fetch data
    $sql = "SELECT * FROM v_ims_purchase_a_master WHERE 1 " . $searchQuery .
