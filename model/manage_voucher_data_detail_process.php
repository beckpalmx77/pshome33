...
// เพิ่มการรับค่า transaction_type และ bank_account
// $transaction_type = $data['transaction_type'] ?? 'expense'; // Removed default 'expense' or any check
...
// Removed the check for $transaction_type === 'expense'
...
try {
    ...
    if ($action === 'ADD') {
        ...
        $stmt = $conn->prepare("INSERT INTO ims_payment_voucher
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        // Removed 'transaction_type' from the VALUES list
        ...
        $stmt = $conn->prepare("UPDATE ims_payment_voucher
            ...
            WHERE doc_no = ?");
        // Removed 'transaction_type = ?' from the SET list
