...
def generate_pdf():
    ...
    # Document Setup (A4, 1.5cm margins)
    ...
    # Metadata Box (Table)
    metadata_data = [
        [thai_text("<b>ประเภทระบบ:</b>"), thai_text("Web Application (LINE OA & LIFF Integration)")],
        [thai_text("<b>เทคโนโลยีหลัก:</b>"), thai_text("Python, FastAPI, DeepFace, Sentence-Transformers, PostgreSQL")],
        [thai_text("<b>ผู้พัฒนา:</b>"), thai_text("Antigravity AI Assistant")],
        [thai_text("<b>เอกสาร:</b>"), thai_text("แนวคิดระบบ สถาปัตยกรรม และขั้นตอนการดำเนินงาน (Concept & Architecture Design)")]
    ]
    ...
    arch_data = [
        [thai_text("<b>ส่วนประกอบ (Component)</b>"), thai_text("<b>หน้าที่การทำงาน (Responsibility)</b>")],
        [thai_text("LINE OA & Webhook"), thai_text("เป็นช่องทางรับส่งข้อความหลัก ส่งข้อมูลกิจกรรมจากห้องแชท LINE ไปยังเซิร์ฟเวอร์หลังบ้าน")],
        [thai_text("LINE LIFF (Frontend)"), thai_text("หน้าจอเว็บฝังใน LINE สำหรับหน้ากรอกฟอร์ม สมัครสมาชิก และเรียกใช้กล้องหน้าสำหรับสแกนใบหน้า")],
        [thai_text("FastAPI Backend"), thai_text("ระบบประมวลผลหลัก (API Gateway) จัดการสิทธิ์การใช้งาน บันทึกข้อมูล และประสานงานกับโมดูล AI")],
        [thai_text("AI Engines"), thai_text("1) Face Verification (ประมวลรูปภาพด้วย DeepFace)<br/>2) Matching Engine (จับคู่ข้อความด้วย Sentence-Transformers)")],
        [thai_text("Database (PostgreSQL)"), thai_text("จัดเก็บข้อมูลติวเตอร์ ข้อมูลนักเรียน รวมถึงเวกเตอร์ของใบหน้าและประวัติติวเตอร์ (pgvector)")],
    ]
