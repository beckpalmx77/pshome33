...
if ($_POST["action"] === 'GET_DATA') {
    ...
    $sql_get = "SELECT * FROM v_ims_house_pet "
        . " WHERE v_ims_house_pet.id = " . $id;
...
if ($_POST["action"] === 'SEARCH') {
    ...
    if ($_POST["house_number"] !== '') {
        ...
        $sql_find = "SELECT * FROM ims_house_pet WHERE house_number = '" . $house_number . "'";
...
if ($_POST["action"] === 'DELETE') {
    ...
    $sql_find = "SELECT * FROM ims_house_pet WHERE id = " . $id;
    $nRows = $conn->query($sql_find)->fetchColumn();
    if ($nRows > 0) {
        try {
...
## Total number of records without filtering
    $stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM v_ims_house_pet WHERE 1=1 " . $where_house_number );
    $stmt->execute();
    $records = $stmt->fetch();
    $totalRecords = $records['allcount'];

## Total number of records with filtering
    $stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM v_ims_house_pet WHERE 1=1 " . $where_house_number . $searchQuery);
...
## Fetch records
    ...
    $sql_get_date = "SELECT * FROM v_ims_house_pet WHERE 1=1 " . $where_house_number . $searchQuery . " LIMIT :limit,:offset";
