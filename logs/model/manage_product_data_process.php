...
if ($_POST["action"] === 'SEARCH') {
    ...
    if ($_POST["product_name"] !== '') {
        ...
        $sql_find = "SELECT * FROM ims_products WHERE product_name = '" . $product_name . "'";
...
if ($_POST["action"] === 'ADD') {
    if ($_POST["product_name"] !== '') {
        $product_id = "U-" . sprintf('%04s', LAST_ID($conn, "ims_products", 'id'));
        $product_name = $_POST["product_name"];
        $status = $_POST["status"];
        $sql_find = "SELECT * FROM ims_products WHERE product_name = '" . $product_name . "'";
        $nRows = $conn->query($sql_find)->fetchColumn();
        if ($nRows > 0) {
            ...
            $sql = "INSERT INTO ims_products(product_id,product_name,status) VALUES (:product_id,:product_name,:status)";
...
if ($_POST["action"] === 'UPDATE') {
    ...
    if ($_POST["product_name"] != '') {
        ...
        $nRows = $conn->query($sql_find)->fetchColumn();
        if ($nRows > 0) {
            $sql_update = "UPDATE ims_products SET product_id=:product_id,product_name=:product_name,status=:status            
            WHERE id = :id";
